export default class WorldState {   // playerCamera
    ...
    near() {
        if (this.next()[0] < 0 || this.next()[0] >= this.maze.array.length || this.next()[1] < 0 || this.next()[1]  >= this.maze.array[0].length) return 2;
    ...
    far() {
        ...
        if (farI < 0 || farI >= this.maze.array.length || farJ < 0 || farJ >= this.maze.array[0].length) return 1;
    ...
    left() {
        ...
        return this.maze.array[this.player.pos[0] + this.player.dirs[i - 1][0]][this.player.pos[1] + this.player.dirs[i - 1][1]];
    }

    right() {
        let i = this.indexOf(this.player.dir);
        return this.maze.array[this.player.pos[0] + this.player.dirs[(i + 1) % this.player.dirs.length][0]][this.player.pos[1] + this.player.dirs[(i + 1) % this.player.dirs.length][1]];
    }
    ...
    farLeft() {
        ...
        if (farLeftI < 0 || farLeftI >= this.maze.array.length || farLeftJ < 0 || farLeftJ >= this.maze.array[0].length) return 1;
    ...
    farRight() {
        ...
        if (farRightI < 0 || farRightI >= this.maze.array.length || farRightJ < 0 || farRightJ >= this.maze.array[0].length) return 1;
        
        return this.maze.array[this.next()[0] + this.player.dirs[(i + 1) % this.player.dirs.length][0]][this.next()[1] + this.player.dirs[(i + 1) % this.player.dirs.length][1]];
    }
